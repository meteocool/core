  ...
  function lightningChartCanvas(elem) {
    ...
    chart = new Chart(lightningCanvas.getContext('2d'), {
      ...
      data: {
        labels: Array(30)
          .fill(null)
          .map((_, i) => (i === 30 - 1 ? 'now' : `-${30 - i} min`)),
        datasets: [
          {
            data: [
              222, 151, 141, 184, 155, 125, 296, 148, 199, 151, 144, 219, 133, 158, 184, 194, 166, 140, 166, 125, 123, 154, 115, 158, 207,
              117, 193, 113, 215, 164,
            ],
            backgroundColor: Array(30)
              .fill(null)
              .map((_, i) => LightningColors[Math.min(30 - i - Math.max(-20 * (30 - i), -30), LightningColors.length - 1)]),
            datalabels: {
              display: false,
            },
          },
